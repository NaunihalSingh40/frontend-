  ...
  const addLeave = async () => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        url = `http://localhost:8080/admin/leaves/${leaves[currentEditIndex]._id}`;
        method = "PUT";
      } else {
        console.log("Adding new leave...");
        url = "http://localhost:8080/admin/leaves/add";
        method = "POST";
      }
      ...
      if (isEditing) {
        ...
        updatedLeaves[currentEditIndex] = result.leave; // Ensure API returns updated leave data
        setLeaves(updatedLeaves);
      } else {
        setLeaves([...leaves, result.leave]); // Ensure API returns the new leave
      }
